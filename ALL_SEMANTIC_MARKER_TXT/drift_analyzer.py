...
from .drift_metrics import (
    calculate_all_drift_metrics, HomeBaseMetrics, DensityMetrics,
    VariabilityMetrics, RiseRateMetrics
)
...
class DriftAnalysisResult:
    ...
    Attributes:
        ...
        drift_metrics: Neue erweiterte Drift-Metriken (Home Base, Density, Variability, Rise Rate)
...
class CoSDAnalyzer:
    ...
    def analyze_drift(
        ...
        # Phase 5: Erweiterte Drift-Metriken
        drift_metrics = calculate_all_drift_metrics(drift_vectors)
    ...
    def _assess_drift_risk(
        ...
        # 5. Neue Drift-Metriken
        if drift_metrics and 'home_base' in drift_metrics:
            # Home Base Stabilität (invers zu Risiko)
            stability_score = drift_metrics['home_base']['stability_score']
            home_base_risk = 1.0 - stability_score
            risk_factors['home_base'] = home_base_risk
            
            # Density Risiko (hohe Dichte = höheres Risiko)
            density_score = drift_metrics['density']['marker_density']
            density_risk = min(1.0, density_score / 10.0)  # Normalisiert
            risk_factors['density'] = density_risk
            
            # Variability Risiko (hohe Variabilität = höheres Risiko)
            variance_score = drift_metrics['variability']['variance_score']
            risk_factors['variability'] = variance_score
            
            # Rise Rate Risiko (hohe Anstiegsrate = höheres Risiko)
            rise_rate = abs(drift_metrics['rise_rate']['average_rise_rate'])
            rise_rate_risk = min(1.0, rise_rate / 0.1)  # Normalisiert
            risk_factors['rise_rate'] = rise_rate_risk
        else:
            risk_factors['home_base'] = 0.0
            risk_factors['density'] = 0.0
            risk_factors['variability'] = 0.0
            risk_factors['rise_rate'] = 0.0
    ...
    def _generate_recommendations(
        ...
        # Neue Metriken-basierte Empfehlungen
        if 'home_base' in risk_factors and risk_factors['home_base'] > 0.7:
            recommendations.append("Instabiler Grundzustand erkannt - emotionale Basis schwankt stark")
        
        if 'density' in risk_factors and risk_factors['density'] > 0.7:
            recommendations.append("Hohe Marker-Dichte - intensive semantische Aktivität, erhöhte Aufmerksamkeit erforderlich")
        
        if 'variability' in risk_factors and risk_factors['variability'] > 0.7:
            recommendations.append("Hohe Variabilität - unvorhersagbare semantische Schwankungen")
        
        if 'rise_rate' in risk_factors and risk_factors['rise_rate'] > 0.7:
            recommendations.append("Schnelle Anstiegsrate - emotionale Intensität steigt rasch an")
